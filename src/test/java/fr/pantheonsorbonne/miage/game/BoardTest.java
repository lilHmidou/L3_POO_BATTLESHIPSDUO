...
public class BoardTest {
    ...
    public void testBoardInitialization() {
        ...
        // Vérifier que le tableau de cellules est correctement initialisé
        ...
        // Vérifier que la liste des bateaux est initialisée mais vide
        ...
        // Vérifier que la liste des coordonnées disponibles est correctement initialisée
        assertNotNull(board.getAvailableCoordinates(), "La liste des coordonnées disponibles ne doit pas être null");
        assertEquals(100, board.getAvailableCoordinates().size(), "Il devrait y avoir 100 coordonnées disponibles à l'initialisation");
    }
    ...
    void testGetBoatsConfigConsistency() {
        ...
        for (int i = 0; i < boatsConfig.length; i++) {
            ...
            assertTrue(boatsConfig[i].length >= 3); // Vérifier qu'il y a au moins l'ID, le nom, et la taille
        }
    }

    @Test
    public void testBoardWithConfigBoats() {
        Player player = new Player(); // Utilisez la classe Player pour placer les bateaux
        player.placeBoats(); // Cette méthode place les bateaux en fonction de la configuration
        Board board = player.getBoard();
        
        Boat[] boats = board.getBoats();
        assertNotNull(boats, "Les bateaux ne doivent pas être null");
        assertEquals(Config.getNbBoats(), boats.length, "Le nombre de bateaux sur le plateau doit correspondre à la configuration");

        // Vérifier que chaque bateau a été placé selon la configuration
        for (int i = 0; i < boats.length; i++) {
            Boat boat = boats[i];
            String[] boatConfig = Config.getBoatsConfig(i);
            assertEquals(Integer.parseInt(boatConfig[0]), boat.getId(), "L'ID du bateau doit correspondre à la configuration");
            assertEquals(boatConfig[1], boat.getName(), "Le nom du bateau doit correspondre à la configuration");
            // ... Autres vérifications selon la configuration
        }
    }
}
